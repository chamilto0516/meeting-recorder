...
class SummaryOutputTests(unittest.TestCase):
    ...
    def test_game_mode_generates_and_saves_two_documents(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(result, [
                ("dm-continuity-brief.md", responses[0]),
                ("player-recap.md", responses[1]),
            ])
            self.assertIn("DM Continuity Brief", call_llm.call_args_list[0].args[0])
            self.assertIn("Player Recap", call_llm.call_args_list[1].args[0])
    ...
    def test_game_mode_reduces_a_long_transcript_before_final_outputs(self) -> None:
        ...
        with patch.object(summarize, "_call_llm", return_value="compact notes") as call_llm:
            summarize.summarize_mode(long_transcript, cfg, mode)
        prompts = [call.args[0] for call in call_llm.call_args_list]
        self.assertGreater(len(call_llm.call_args_list), 2)
        self.assertIn("Open Threads for Prep", prompts[0])
        self.assertIn("DM Continuity Brief", prompts[-2])
        self.assertIn("Player Recap", prompts[-1])
